...
public class LibroInteractivo : MaterialDigital, IPrestable, IMultimedia
{
    ...
    {
        if (!new[] { "iOS", "Android", "Web" }.Contains(plataforma))
            throw new ArgumentException("La plataforma debe ser iOS, Android o Web");
    ...
    public override void ValidarIntegridad()
    {
        ...
        if (RecursosMultimedia < 0)
            throw new ArgumentException("Los recursos multimedia no pueden ser negativos");
    }
    ...
    public void Prestar(UsuarioBase usuario)
    {
        if (!VerificarDisponibilidad())
            throw new InvalidOperationException("El libro interactivo no está disponible");
    ...
    {
        if (!EstaPrestado)
            throw new InvalidOperationException("El libro interactivo no está prestado");
        EstaPrestado = false;
